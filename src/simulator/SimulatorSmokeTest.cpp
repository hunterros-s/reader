...
class SimulatorSmokeTest {
 ...
 private:
  ...
  static bool enabled() { return std::getenv("CROSSPOINT_SIMULATOR_SMOKE_TEST") != nullptr; }

  static int pageTurnCount() {
    const char* raw = std::getenv("CROSSPOINT_SIMULATOR_SMOKE_PAGE_TURNS");
  ...
  void tickImpl() {
    ...
    switch (step) {
      ...
      case SmokeStep::Sleep: {
        const char* bookPath = std::getenv("CROSSPOINT_SIMULATOR_SMOKE_BOOK");
        if (bookPath == nullptr || bookPath[0] == '\0') {
          LOG_INF("SMOKE", "Skipping Reader step; CROSSPOINT_SIMULATOR_SMOKE_BOOK is not set");
          step = SmokeStep::Done;
          break;
        }
